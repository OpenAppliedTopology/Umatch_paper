...
/// Get the transpose of a CSM matrix with the same major dimension as the input square matrix
///
/// # Parameters
/// - `matrix`: A square CSM matrix whoes minor index are integers
/// - `nummin`: Number of columns of the input sparse matrix
pub fn transpose<SnzVal: Clone + PartialEq + Debug>(nummin: usize, matrix: &CSM<usize, SnzVal>) -> CSM<usize, SnzVal> {

    let nummaj = nummin;
    let mut majptr = vec![0; nummaj+2];
    let num_val = matrix.minind.len();
    let mut minind = vec![0; num_val];
    let mut snzval = vec![matrix.snzval[0].clone(); num_val];

    // count per column
    for ii in 0..num_val {
        majptr[matrix.minind[ii]+2] += 1;
    }

    for ii in 2..(nummaj+2) {
        majptr[ii] += majptr[ii-1];
    }

    // perform the main part
    for major_index in 0..matrix.nummaj {
        for jj in matrix.majptr[major_index]..matrix.majptr[major_index+1] {
            // calculate index of the transposed matrix at which we should place current
            // snzval/minind, and at the same time build the final majptr
            let minor_index = matrix.minind[jj];
            let new_index = majptr[minor_index + 1];
            snzval[new_index] = matrix.snzval[jj].clone();
            minind[new_index] = major_index;
            majptr[minor_index + 1] += 1;
        }
    }
    ...
    return CSM {
        ringmetadata: matrix.ringmetadata.clone(),
        nummaj,
        majdim: matrix.majdim.clone(),
        majptr,
        minind,
        snzval
    };

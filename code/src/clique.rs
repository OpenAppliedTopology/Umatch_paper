...
pub fn pts_to_dismat<FilVal:Real + std::ops::Sub<Output = FilVal>>(pts: Vec<Vec<FilVal>>) -> Vec<Vec<FilVal>> {
    let size = pts.len();
    let mut dismat:Vec<Vec<FilVal>> = Vec::new();
    let zero = pts[0][0]-pts[0][0];

    for row in 0..size {
        let mut temp_row = vec![zero; size];
        for col in 0..size {
            if col < row { temp_row[col] = dismat[col][row]; }
            if col > row {
                let mut dist:FilVal = zero;
                for ind in 0..pts[0].len() {
                    dist = dist + (pts[row][ind]-pts[col][ind]).powi(2);
                }
                temp_row[col] = dist.sqrt();
            }
        }
        dismat.push(temp_row);
    }
    return dismat;
}
...
SnzVal: Neg<Output = SnzVal> + Clone
{
	...
	fn min_itr( &self, minkey: &Simplex<FilVal> ) -> Box<dyn Iterator<Item=(Simplex<FilVal>, SnzVal)> + '_> {
        if self.majordimension == MajorDimension::Row {
            ...
            let mut vertices = minkey.vertices.clone();
            let first_vert = vertices[0];
            vertices.insert(0,0);
            vertices.shrink_to_fit();
            Box::new(CofacetIter{
                clique: &self,
                next_cofacet_vertices: vertices,
                simplex_filvalue: minkey.filvalue.clone(),
                insertion_location: 0,
                candidate_location: 0,
                coeff: self.ringmetadata.identity_multiplicative.clone(),
                first_vert,

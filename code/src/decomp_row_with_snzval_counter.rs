...
pub fn decomp_row_with_snzval_counter<MajKey, MinKey, SnzVal, Matrix>(
    matrix:             &Matrix,
    maj_to_reduce:      &mut Vec<MajKey>,
) -> (CSM<usize, SnzVal>, Indexing<MinKey, MajKey>, usize) where
...
Matrix: SmOracle<MajKey, MinKey, SnzVal>
{
    ...
    // Set up counter for structural non-zero entries in rowoper
		...
		while let Some(Reverse(minkey)) = heap_reduced.pop(){
            if let Some(leading_entry) = hash_reduced.remove(&minkey){
                if matrix.ring().is_0(&leading_entry) { continue; }
                    ...
                    rowoper.push_snzval(rowoper.nummaj, one.clone());
                    rowoper.append_maj(&mut hash_rowoper.clone());
                    break;
                }
            }
        }
        // update counter
        counter += hash_rowoper.len();
	}
    ...
    return (rowoper, indexing, counter);
}
